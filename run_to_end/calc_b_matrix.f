...
C		Converts between internal coordinates intramolecular vibrations (OH/OD stretch to HOH/DOD bends) 
C		to Cartesian coordinates for nmono sized water clusters via 5 point finite differencing.
C		Inputs: 
C		coord = 3*natoms Cartesian coordinates in units of bohr
C		q_f_r = 3*nmono internal displacements describing all of the intramolecular vibrations of each water 
C		cluster. Displacements are determined by the drift term in the trial wave function.
C		nmono = number of water molecules in the cluster of interest
C		Outputs:
C		q_f_cart = 3*natoms Cartesian displacements converted from the internal coordinate displacement.
C		first_trans = first derivatives that describe transformation from internals to Cartesians.
C		second_trans = second derivatives that describe transformation from internals to Cartesians.
        pi = dacos(-1.d0)
        ip = 0
        dx = 0.0001
